...
import sqlite3, json, uuid, os, html, secrets, sys
...
DB_PATH = os.environ.get('DB_PATH', os.path.join(os.path.dirname(os.path.abspath(__file__)), 'eve_cafe.db'))
...
def generate_token(admin_id):
    ...
    return s.dumps(admin_id)

def verify_token(token):
    s = URLSafeTimedSerializer(app.secret_key)
    try:
        admin_id = s.loads(token, max_age=86400) # 24 hours
        return admin_id
    except:
        return None
...
def init_db():
    ...
    # Check if estimated_time column exists in orders, if not add it
    cursor = conn.cursor()
    cursor.execute("PRAGMA table_info(orders)")
    columns = [row[1] for row in cursor.fetchall()]
    if 'estimated_time' not in columns:
        conn.execute("ALTER TABLE orders ADD COLUMN estimated_time TEXT DEFAULT ''")
        conn.commit()
    ...
    if not conn.execute("SELECT 1 FROM categories LIMIT 1").fetchone():
        ...
        for i in range(1, 13):
            conn.execute("INSERT OR IGNORE INTO tables (number,status,seats) VALUES (?,?,?)",
                         (i, 'free', 4))
        # Admin user
        default_pass = os.environ.get('ADMIN_PASSWORD', 'thisisbusiness')
        conn.execute(
            "INSERT OR IGNORE INTO admins (id,username,password) VALUES (?,?,?)",
            (str(uuid.uuid4()), 'AD.com', generate_password_hash(default_pass)))
    conn.commit()
    conn.close()
...
def place_order():
    ...
    notes      = html.escape(str(notes_raw))
    chips_raw  = body.get('chips') or []
    chips      = [html.escape(str(c)) for c in chips_raw if c is not None] if isinstance(chips_raw, list) else []
...
@app.route('/api/orders/<order_id>/estimated_time', methods=['PATCH'])
@admin_required
def update_estimated_time(order_id):
    body = request.get_json(silent=True) or {}
    est_time = body.get('estimated_time', '').strip()
    est_time = html.escape(str(est_time))
    
    d = db()
    o = d.execute("SELECT * FROM orders WHERE id=?", (order_id,)).fetchone()
    if not o:
        return err('Order not found', 404)
        
    d.execute("UPDATE orders SET estimated_time=?, updated_at=? WHERE id=?", 
              (est_time, now_iso(), order_id))
    d.commit()
    return jsonify({'order_id': order_id, 'estimated_time': est_time})
...
def login():
    ...
    d = db()
    admin = d.execute("SELECT * FROM admins WHERE username=?", (username,)).fetchone()
    if not admin:
    ...
    if env_pass and password == env_pass:
        if not check_password_hash(password_hash, password):
            ...
            d.execute("UPDATE admins SET password=? WHERE id=?", (password_hash, admin['id']))
            d.commit()
...
def analytics():
    ...
    # Calculate today's revenue (UTC date match)
    today_utc = datetime.now(timezone.utc).strftime('%Y-%m-%d')
    today_revenue = d.execute(
...
# ─────────────────────────────────────────
# STARTUP — runs under both gunicorn & direct
# ─────────────────────────────────────────
init_db()  # safe — CREATE IF NOT EXISTS, runs on every worker start
